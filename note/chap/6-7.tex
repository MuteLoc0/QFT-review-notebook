...
\begin{equation*}
    \textstyle D_F(x,y) \equiv \braket{0|T\{\phi_0(x)\phi_0(y)\}|0} = \int \frac{\dif ^4 k}{(2\pi)^4} \frac{\ramuno}{k^2- m^2 + \ramuno\epsilon}\eu^{\ramuno k (x-y)}
\end{equation*}
Schwartz书中\concept{Schwinger-Dyson equations}{
\begin{equation*}
    \textstyle(\square_x + m^2)\braket{\phi_x \phi_1 \cdots \phi_n} = \braket{\mathcal{L}'_\text{int}[\phi_x]\phi_1 \cdots \phi_n} - \ramuno\hbar \sum_j \delta^4(x-x_j)\braket{\text{没有}\phi_j}
\end{equation*}}
...
Wick定理，时序算符和收缩的来源：
\begin{equation*}
    \textstyle\braket{\Omega|T\{\phi(x_1)\cdots \phi(x_n)\}|\Omega} = \frac{\braket{0|T\{\phi_0(x_1)\cdots \phi_0(x_n)\eu^{\ramuno\int \dif^4x\,\mathcal{L}_\text{int}[\phi_0]}\}|0}}{\braket{0|T\{\eu^{\ramuno\int \dif^4x\,\mathcal{L}_\text{int}[\phi_0]}\}|0}}
\end{equation*}
...
\concept{位置空间的费曼规则}{}以$\scriptstyle\mathcal{L}_\text{int} = -\frac{\lambda}{4!}\phi^4$理论为例------\vspace{-1em}
\begin{itemize}[nosep]
...
\end{equation*}
\begin{align*}
    \mathcal{L} &= -\frac{1}{4}F_{\mu\nu}^2 + |D_\mu \phi|^2 - m^2 |\phi|^2\quad (\text{where }D_\mu = \partial_\mu + \ramuno A_\mu)\\
    &= -\frac{1}{4}F_{\mu\nu}^2 - \phi^*(\square + m^2)\phi - \ramuno e A_\mu [\phi^*(\partial_\mu\phi) - (\partial_\mu\phi^*)\phi] + e^2 A_\mu^2|\phi|^2.
\end{align*}
\vspace{-1em}
\begin{itemize}[nosep]
    \item 复标量传播子：$\scalebox{0.7}{\feynmandiagram [horizontal=a to b, node distance=1cm,every edge/.style={draw, thick}] {a --[dashed, fermion, edge label = $p$] b};}=\textstyle \frac{\ramuno}{p^2 - m^2 +\ramuno\epsilon}$
    \item 光子传播子：$\scalebox{0.7}{\feynmandiagram [horizontal=a to b, node distance=1cm,] {a --[photon, edge label = $p$] b};}= \textstyle \frac{-\ramuno}{p^2 +\ramuno\epsilon}\left[g_{\mu\nu} - (1-\xi)\frac{p_\mu p_\nu}{p^2}\right]$，费曼规范下$\xi=1$，郎道规范$\xi = 0$
    \item $-\ramuno e A_\mu[\cdots]$项顶点：标量QED顶点给出的是粒子流箭头(即线上箭头)指向外侧的粒子的动量之和减去箭头指向内侧的粒子的动量的$-\ramuno e$倍。$\scalebox{0.4}{\feynmandiagram [horizontal=a to b, node distance=1cm,baseline=a to b,every edge/.style={draw, thick}] {a[particle = $e^-$] --[dashed, fermion, momentum' =$p_1$] b,b --[dashed, fermion,momentum' = $p_2$] c[particle = $e^-$], b --[photon] d};}= -\ramuno e (p_1 +p_2)$
  ...
  \end{feynman}
\end{tikzpicture}}= \textstyle 2\ramuno e^2 g_{\mu\nu}$
\end{itemize}
Example \concept{M\o ller Scattering}{$e^- e^- \to e^- e^-$。}由于光子不能带有电荷，只有$t,u$通道存在。图象可以看前面，把外线换为虚线、内线换为波浪线即可
\begin{align*}
    &\scriptstyle\ramuno \mathcal{M}_t = (-\ramuno e)^2(p_1^\mu + p_3^\mu)\frac{-\ramuno g_{\mu\nu} }{k^2}(p_2^\nu +p_4^\nu) = \ramuno e^2 \frac{(p_1^\mu + p_3^\mu)(p_2^\mu + p_4^\mu)}{t},\\
    &\scriptstyle\ramuno \mathcal{M}_u=e^2\frac{(p_1^\mu + p_4^\mu)(p_2^\mu + p_3^\mu)}{u}.
\end{align*}
在高斯单位制下$\alpha = \frac{e^2}{4\pi}$，于是M\o ller Scattering的微分散射截面是
\begin{equation*}
    \textstyle\frac{\dif \sigma}{\dif \Omega} = \frac{1}{64\pi^2 E_{\text{cm}}^2} |\mathcal{M}|^2  = \frac{e^4}{64\pi^2 E^2_\text{CM}}\left[\frac{s-u}{t}+\frac{s-t}{u}\right]^2.
\end{equation*}
