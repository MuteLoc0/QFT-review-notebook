...
\begin{center}
\begin{tabular}{c|c|c}
    \scalebox{0.5}{\begin{tikzpicture}[baseline=(current bounding box.center)]\begin{feynman}
            \vertex (c);
...
\end{equation}
\begin{equation}
    \mathcal{L} = -\frac{1}{4}F_{\mu\nu}F^{\mu\nu} - \phi^*(\square + m^2)\phi - \ramuno e A_\mu [\phi^*(\partial_mu\phi) - (\partial_\mu\phi^*)\phi] + e^2 A_\mu A^\mu |\phi|^2.
\end{equation}