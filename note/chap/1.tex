...
\begin{align*}
    \scriptstyle  \ramuno \overline{\psi}(x) \not{\partial} \psi(x) \to \ramuno \overline{\psi}(x) \not{\partial} \psi(x) + \overline{\psi}(x) \gamma^\mu \psi(x) \partial_\mu \alpha(x) \\
    \scriptstyle \psi(x_1) \overline{\psi}(x_2) \to \eu^{-\ramuno \alpha(x_1)} \eu^{\ramuno \alpha(x_2)} \psi(x_1) \overline{\psi}(x_2)
\end{align*}    
...
\end{align*}
正则对易关系：$\scriptstyle [a_{p}^{r},a_{q}^{r\dagger}]=(2\pi)^3 \delta^{rs} \delta^{3}(p-q)$。$\scriptstyle \epsilon_r(\bm{p}) \cdot \epsilon_s(\bm{p})=\delta_{rs}$，$\scriptstyle \sum_{r=1}^2 \epsilon_r^i(\bm{p}) \epsilon_r^j(\bm{p}) = \delta^{ij} - \frac{p^i p^j}{|\bm{p}|^2}$
最终得到：
\begin{equation*}
