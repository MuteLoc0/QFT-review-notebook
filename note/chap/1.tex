...
\end{equation*}
定义Noether Current$j^\mu(x) = \frac{\partial \mathcal{L}}{\partial(\partial_\mu \phi)} \Delta \phi - \mathcal{J}^\mu(x)
$\\
由运动方程可证该流是守恒的：
\begin{equation*}
...
\begin{align*}
   D_{ij}& = D_F(x_i - x_j) 
          ...
          &= \int \frac{\dif^4 p}{(2\pi)^4} \frac{\ramuno}{p^2 - m^2 + \ramuno\epsilon} \eu^{-\ramuno p \cdot (x_i - x_j)} 
\end{align*}
...
把$J(y) = \int \dif^4 x \, \delta^{(4)}(x-y) J(x)$ 和$ \frac{\delta J(x)}{\delta J(y)} = \delta^{(4)}(x-y)$合并，有：
   \begin{equation*}
...
\begin{equation*}
    \ramuno \mathcal{M}_2  = \frac{(-\ramuno \lambda)^2}{2} \int \frac{\dif^4 k}{(2\pi)^4} \frac{\ramuno}{k^2 + \ramuno \epsilon} \frac{\ramuno}{(p-k)^2 + \ramuno \epsilon}
\end{equation*}
...
\end{equation*}
接着使用Pauli–Villars formula$ \int \frac{\dif^4 k}{(2\pi)^4} \frac{1}{(k^2 - \Delta + \ramuno\varepsilon)^2} = - \frac{\ramuno}{16\pi^2} \ln \frac{\Delta}{\Lambda^2}.$，并假设m=0，得到：
\begin{equation*}
