...
\begin{align*}
    \bm{A}(x) &= \int \frac{\dif^3 p}{(2\pi)^3 \sqrt{2|\bm{p}|}} \sum_{r=1}^2 \bm{\epsilon}_r(\bm{p}) \left[ a_{\bm{p}}^r \eu^{-\ramuno p \cdot x} + a_{\bm{p}}^{r\dagger} \eu^{\ramuno p \cdot x} \right] \\
    \bm{E}(x) &= \int \frac{\dif^3 p}{(2\pi)^3} (-\ramuno)\sqrt{\frac{|\bm{p}|}{2}} \sum_{r=1}^2 \bm{\epsilon}_r(\bm{p}) \left[ a_{\bm{p}}^r \eu^{-\ramuno p \cdot x} - a_{\bm{p}}^{r\dagger} \eu^{\ramuno p \cdot x} \right]
\end{align*}
正则对易关系：$[a_{p}^{r},a_{q}^{r\dagger}]=(2\pi)^3 \delta^{rs} \delta^{3}(p-q)$\\
$ \epsilon_r(\bm{p}) \cdot \epsilon_s(\bm{p})=\delta_{rs} $\quad
   $ \sum_{r=1}^2 \epsilon_r^i(\bm{p}) \epsilon_r^j(\bm{p}) = \delta^{ij} - \frac{p^i p^j}{|\bm{p}|^2} $
最终得到：
\begin{equation*}
    [A_i(\bm{x}), E_j(\bm{y})] = \ramuno \left( \delta_{ij} - \frac{\partial_i \partial_j}{\nabla^2} \right) \delta^{(3)}(\bm{x}-\bm{y}) = \ramuno \int \frac{\dif^3 p}{(2\pi)^3} \left( \delta_{ij} - \frac{p_i p_j}{|\bm{p}|^2} \right) \eu^{\ramuno \bm{p} \cdot (\bm{x}-\bm{y})}
\end{equation*}
\begin{equation*}
    H = \int \frac{\dif^3 p}{(2\pi)^3} |\bm{p}| \sum_{r=1}^2 a_{\bm{p}}^{r\dagger} a_{\bm{p}}^r
\end{equation*}

\begin{equation*}
    D_{ij}(x-y) = \bra{0} T A_i(x) A_j(y) \ket{0} = \int \frac{\dif^4 p}{(2\pi)^4} \frac{\ramuno}{p^2 + \ramuno\epsilon} \left( \delta_{ij} - \frac{p_i p_j}{|\bm{p}|^2} \right) \eu^{-\ramuno p \cdot (x-y)}
\end{equation*}
