...
$\gamma$矩阵性质：
    \begin{align*}
        ...
        \scriptstyle\gamma^\mu &\scriptstyle= \left(\begin{smallmatrix}
            0 &\sigma^\mu \\
            \bar{\sigma}^\mu &0
        \end{smallmatrix}\right),\quad\sigma_x = \left( \begin{smallmatrix} 0 & 1 \\ 1 & 0 \end{smallmatrix} \right), \quad 
\sigma_y = \left( \begin{smallmatrix} 0 & -i \\ i & 0 \end{smallmatrix} \right), \quad 
...
\begin{equation*}
        \scriptstyle(\ramuno \gamma^\mu \partial_\mu - m)\psi = \begin{pmatrix}
            \scriptstyle-m &\scriptstyle \ramuno \sigma\cdot \partial\\
            \scriptstyle\ramuno \bar{\sigma}\cdot \partial &\scriptstyle -m
        \end{pmatrix}\begin{pmatrix}
            \scriptstyle\psi_L\\
            \scriptstyle\psi_R
        \end{pmatrix} = 0.
\end{equation*}
据$\scriptstyle\varLambda_s^{-1}\gamma^\mu \varLambda_s = {(\varLambda_v)^\mu}_\nu \gamma^\nu$在洛仑兹变换下不变。其平面波解$\scriptstyle\phi = u_s(p)\eu^{-\ramuno p x}$和$\scriptstyle v_s(p)\eu^{\ramuno p x}$，$s=1,2$描述偏振方向，其中
\begin{equation*}
